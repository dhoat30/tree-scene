...
export async function generateMetadata(props, parent) {
  ...
  if (data.length > 0) {
    ...
    return {
      ...
      alternates: {
        canonical: "https://treescene.co.nz/about-us",
      },
      openGraph: {
        title: seoData.title,
        description: seoData.description,
        url: "https://treescene.co.nz/about-us",
...
export default async function Contact(props) {
  ...
  const [postData, allPosts, options, serviceJobs] = await Promise.all([
    getSinglePostData("about-us", "/wp-json/wp/v2/pages"),
    getAllPosts("wp-json/wp/v2/industrial-cleaning"),
    getOptions(),
    getServiceJobsWithClients(),
  ]);
