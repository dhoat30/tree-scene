...
export async function generateMetadata(props, parent) {
  ...
  if (data.length > 0) {
    ...
    return {
      ...
      alternates: {
        canonical: `https://treescene.co.nz/services/${slug}`,
      },
      openGraph: {
        title: seoData.title,
        description: seoData.description,
        url: `https://treescene.co.nz/services/${slug}`,
...
export default async function Contact(props) {
  ...
  const [postData, options, serviceJobs] = await Promise.all([
    getSinglePostData(slug, "/wp-json/wp/v2/service"),
    getOptions(),
    getServiceJobsWithClients(),
  ]);
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "Service",
            name: postData[0]?.title?.rendered,
            description: postData[0]?.yoast_head_json?.description,
            provider: {
              "@type": "LocalBusiness",
