...
export default function MultipartForm({
  ...
  const handleNext = () => {
    ...
    if (
      ...
      !currentField.validation(formData[currentField.id])
    ) {
  ...
  if (multipartFormData[activeStep].id === "address") {
    inputField = (
      <React.Fragment key={multipartFormData[activeStep].id}>
        {!mapsLoaded && (
          <GoogleMapsLoader onLoad={handleLoad} key="google-maps-loader" />
        )}
        {mapsLoaded && (
          <GoogleAutocomplete
            className="mt-16"
            ...
            value={formData.address}
            onChange={(value) =>
              handleChange(multipartFormData[activeStep].id, value, false)
            }
            ...
            helperText={
              multipartFormData[activeStep].errorMessage
    ...
    inputField = (
      <Input
        ...
        onChange={
          multipartFormData[activeStep].type === "chip"
            ? (newValue) =>
                handleChange(
                  multipartFormData[activeStep].id,
                  newValue,
                  multipartFormData[activeStep].multiple,
                )
            : (e) =>
                handleChange(
                  multipartFormData[activeStep].id,
        ...
        <div className="wrapper">
          ...
          <React.Fragment>
            <div className="input-wrapper p-6">
              ...
              <Box
                className="button-wrapper mt-16"
